...
另外，你可以在 `src/data/bounding-box/` 文件夹下查看到 `bbox-*` 文件，这些是用来训练识别 bbox （标注框）的训练与测试文件。由于**模型训练耗时较长且效果不佳**，这些文件正在测试中。`validate_bbox_coords.py` 文件使用来查看训练与测试文件的标注框位置是否正确的，以防止图像放缩导致异常。
